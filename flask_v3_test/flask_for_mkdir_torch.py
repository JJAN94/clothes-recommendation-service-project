...
# print(musinsa['img'],'???????')
with open('model/mu_index_epoch09.json', 'r') as f:
    mu_index = json.load(f)
# mu_search = AnnoyIndex(4096, 'angular')
mu_search = AnnoyIndex(1000, 'angular')
mu_search.load('model/mu_search_epoch09.ann')

with open('model/insta_index_epoch09.json', 'r') as f:
...
insta_search.load('model/insta_search_epoch09.ann')
...
basemodel = torch.load(os.path.join(
    PATH, sorted(os.listdir(PATH))[-1])).to(device)
basemodel.eval()
print(sorted(os.listdir(PATH))[-1])
print(basemodel)
